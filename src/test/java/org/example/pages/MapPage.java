...
public class MapPage extends BasePage {
    ...
    public MapPage enableEventLayers() {
        ...
        // TODO: Необходимо доработать логику выбора события
        // Проблема: После включения слоёв ДТП на карте может не быть событий
        // Возможные причины:
        // 1. В выбранной локации/дате действительно нет ДТП
        // 2. События отображаются с задержкой или требуют дополнительного действия
        if (events.isEmpty()) {
            System.out.println("На карте нет событий");
        } else {
            System.out.println("На карте найдено событий: " + events.size() + ", открываем первое");
            click(events.get(0));
        }
